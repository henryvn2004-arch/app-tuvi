...
- **⏭️ PICK UP NGAY (2026-06-23, branch `claude/meta-channels-messenger-whatsapp`):** PR **#102** (Messenger + WhatsApp + tổng quát hóa DB) đã push, **CI XANH 7/7**, đang để **DRAFT — CHƯA merge** (cố ý). Lý do chưa merge: phải chạy migration `_patches/migration-channels-multiplatform.sql` trong Supabase TRƯỚC (tạo bảng `chat_*` + copy data Telegram) để bot Telegram LIVE liền mạch. **Việc tiếp:** (1) Henry chạy migration → (2) nói "đã chạy migration #102, merge đi" → Claude mark ready + squash-merge → (3) Henry set env Meta trên Vercel + Redeploy + trỏ webhook (chi tiết ở mục "VIỆC TAY HENRY (Messenger/WhatsApp)" bên dưới) → (4) Claude test. **Câu hỏi treo cần Henry chốt:** liên kết ví Lượng từ WEB cho Messenger/WhatsApp (giờ 2 kênh chạy free-cap/ngày, chưa gắn ví) — làm UI liên kết luôn hay để sau?
...
- **⚠️ VIỆC TAY HENRY (để Messenger/WhatsApp chạy thật) — CHƯA xong:** (a) **Chạy migration `_patches/migration-channels-multiplatform.sql` TRƯỚC khi merge/redeploy** (tạo bảng `chat_*` + COPY dữ liệu Telegram sang `platform='telegram'`; KHÔNG đụng bảng `telegram_*` cũ → bot Telegram LIVE liền mạch). (b) **Meta Developer:** tạo App (Business) → add sản phẩm Messenger + WhatsApp. **Messenger:** lấy `MESSENGER_PAGE_ACCESS_TOKEN` (token Page), `MESSENGER_APP_SECRET`, đặt `MESSENGER_VERIFY_TOKEN` (chuỗi tùy ý) → webhook trỏ `https://www.tuviminhbao.com/api/channels/messenger`, subscribe field `messages`. **WhatsApp:** lấy `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_TOKEN` (System User token vĩnh viễn), `WHATSAPP_APP_SECRET`, đặt `WHATSAPP_VERIFY_TOKEN` → webhook `…/api/channels/whatsapp`, subscribe `messages`. (tùy) `MESSENGER_FREE_DAILY`/`WHATSAPP_FREE_DAILY` (mặc định 3), `META_GRAPH_VERSION` (mặc định v21.0). (c) Set env Vercel + Redeploy. (d) Test: nhắn Page/số WhatsApp → bot trả lời, gửi ảnh mặt/nhà, /new. **Lưu ý:** Messenger dev mode chỉ admin/tester nhắn được tới khi App Review duyệt `pages_messaging`; WhatsApp cần Business verification + số WABA.
