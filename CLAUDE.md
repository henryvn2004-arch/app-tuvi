...
- **⏭️ PICK UP NGAY (2026-06-24, chat answer-shape v1 đã MERGE — đang chờ Henry test prod + làm PR2 chip):** PR **#104** **ĐÃ MERGE** vào main (`main` @ commit **56fae5a**). Gồm 3 thứ: (1) **answer-shape v1** — `CHAT_RICH_RULES`+`CHAT_SYSTEM_LASO` (`lib/agent/prompts.ts`) đổi sang **3 lớp** (phán quyết in đậm → 1 mạch dẫn chứng cốt lõi, 1 mạnh+1 yếu, không dàn trải → **MỞ NÚT** nêu đích danh 1 chi tiết thật chưa luận + mời hỏi tiếp); độ dài **130–200 từ** (was 250–600), follow-up 80–140. (2) **Luật vận hạn "đại vận là gốc"** (prompts.ts + `TOOLS_INSTRUCTION` trong `tools.ts`): đại vận là tầng DUY NHẤT có điểm/10 thật; tiểu→nguyệt→nhật phái sinh; đặt vận ngắn trong khung đại vận (đại vận tốt thì sao xấu nhất thời lướt qua); **nguyệt/nhật vận CẤM bịa điểm**, luận theo cung+chính tinh. (3) **Cửa sổ tiểu/nguyệt vận ±5→±10** TÁCH tham số (`tinhTieuVanScores` thêm `windowYears`, `anSaoLaSo` thêm `tieuVanWindow`, `computeLaso` truyền 10) — **client/biểu đồ nến giữ ±5**, chỉ chat server giãn ±10 (smoke OK: client 2021–2031 / server 2016–2036), KHÔNG tốn token LLM. **Việc tiếp NGAY:** (a) Henry test văn phong mới trên **prod** (`/tuvi-chat.html` — gọn hơn? có mồi hỏi tiếp? vận tháng/ngày không bịa điểm?), tinh chỉnh tông qua `app_config.chat.system_prompt` không cần deploy. (b) **PR 2 — chip câu hỏi gợi ý** (bot emit 2–3 câu hỏi tiếp → render nút: web=chip, Telegram=inline keyboard, Messenger=quick replies, WhatsApp=interactive buttons ≤3): CHƯA làm, chờ chốt shape. (c) Phần **trust/retrodiction (#3)** đã bàn nguyên tắc (character-first + trigger phản tỉnh + retrodiction có mốc), CHỐT dial brand rồi mới code.
- **🐞 NỢ KỸ THUẬT (nhỏ, ngoài #104):** `extractLasoContext` (path nhẹ, `lib/agent/prompts.ts` ~dòng 305) còn hardcode `const NAM_XEM = 2027` cho mapping năm→đại vận — sót từ #83. Nên dọn (dùng `currentNamXem()`) vì lệch vận năm ở path nhẹ. Henry đã biết, chưa quyết gộp hay tách PR.
