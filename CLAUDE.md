...
- **⏭️ PICK UP NGAY (2026-06-23, Messenger + WhatsApp đã wiring xong — đang chờ TEST runtime):** PR **#102** (Messenger + WhatsApp + tổng quát hóa DB) **ĐÃ MERGE** vào main (`main` @ commit **5434167**); migration `chat_*` đã chạy trong Supabase. **Meta wiring XONG:** App Business mới (App ID `4355400224733286`), Page Messenger id `122097706839369476`, WhatsApp Phone Number ID `1176714088859217`, số test `+1 555 652-8238`. 8 env đã set Vercel + Redeploy, token Messenger & WhatsApp đều **vĩnh viễn**. Webhook đã trỏ + verify cả 2 (`/api/channels/messenger` + `/whatsapp`, field `messages`, Page subscribed), endpoint test 200. Đã tạo 3 QR deep link (Telegram/Messenger/WhatsApp). **Việc tiếp NGAY:** Henry nhắn test (Messenger từ acc admin, WhatsApp từ số đã add recipients) → Claude soi **runtime log Vercel** xác nhận bot nhận + trả lời. **Phạm vi:** Messenger đang **Development mode** (chỉ admin/tester được trả lời tới khi App Review duyệt `pages_messaging`); WhatsApp đang **số test sandbox** (cần gắn số WABA production để mở cho người thật). **Đã CHỐT:** liên kết ví Lượng cho Messenger/WhatsApp = để SAU → giờ chạy **free-cap/ngày**.
