...
### 🔴 Bước tiếp theo
1. **Merge Sprint 1.6** (fix parity năm xem) khi CI xanh. Sau merge: `git reset --hard origin/main`.
2. **Henry test preview**: `/tuvi-chat.html` → hỏi "năm nay/năm sau", "tháng X/YYYY", tuổi mụ → verify dùng NĂM HIỆN TẠI (không còn 2027); `tra_tieu_van`/`tra_nguyet_van` không báo "ngoài phạm vi" cho năm gần. Sinh Con/Chọn Ngày/Đặt Tên: `scenario.data` là input thô.
