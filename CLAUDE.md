...
- **State (2026-06-23):** Phase 0 + Phase 1 web DONE. **Đã LIVE trên prod:** Pricing 5 Lượng/lượt + signup 25 Lượng (#87, #88); Telegram bot `@tuviminhbao_bot` (#91–93) — chạy free. **Merge hôm nay (chưa LIVE tới khi Henry làm việc tay):** #96 web chat xem ẢNH (tướng mặt/phong thủy, multimodal), #97 Telegram NHỚ lá số theo phiên, #98 Telegram nhận ẢNH, #99 TÁCH lõi kênh chat dùng chung. Branch `claude/tuviminhbao-resume-tf8tcq` đã merge tới #99, branch sạch.
...
- **🔴 BƯỚC TIẾP — Zalo OA:** Henry đang đăng ký Zalo OA (oa.zalo.me, CCCD/GPKD, 2-3 ngày). Khi có access token → viết adapter Zalo theo mẫu Telegram + lõi core (đã chốt cách làm: "tách core chung trước"). Zalo API riêng (của VNG): webhook JSON, send qua OA API, verify appsecret_proof. Messenger+WhatsApp (cùng Meta Graph) làm sau, rất giống nhau. Khi ráp nền tảng thứ 2: tổng quát hóa bảng `telegram_sessions`/`telegram_links` thành đa-nền-tảng (thêm cột `platform`).
- **⚠️ VIỆC TAY HENRY (để Telegram chạy thật trên prod — CHƯA xong):** (a) chạy `_patches/migration-telegram-sessions.sql` (giờ có cột `birth jsonb`) + `_patches/migration-telegram-links.sql` trong Supabase (project `dciwkfdqhhddeymlisey`); (b) env Vercel: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, (tùy) `TELEGRAM_BOT_USERNAME` (mặc định `tuviminhbao_bot`), `TELEGRAM_FREE_DAILY` (mặc định 3) → Redeploy; (c) Claude đăng ký `setWebhook` (kèm secret) rồi test bot: nhớ lá số qua >6 lượt, gửi ảnh mặt/nhà, /new, liên kết ví Lượng.
