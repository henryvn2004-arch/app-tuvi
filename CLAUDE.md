...
- **✅ ĐÃ CHỐT monetize Telegram = hướng 1 (2026-06-22):** gắn user Telegram → ví Lượng dùng CHUNG với web; nạp tiền trên web (PayOS/PayPal → thẳng bank VN). Bỏ Telegram Stars (đường vòng crypto/Fragment, giữ 21 ngày, store cắt 30%, pháp lý crypto VN vùng xám). Mini App (hướng 3) để dành gộp với Zalo. **Đang code trên branch `claude/tuviminhbao-resume-tf8tcq`:** bảng `telegram_links`/`telegram_link_tokens`/`telegram_usage` + `lib/channels/telegramLink.ts` + endpoint `app/api/channels/telegram/link/route.ts` (web sinh token → deep link `t.me/<bot>?start=<token>` → bot `/start <token>` gắn ví) + paywall trong adapter Telegram (đã link → trừ ví; chưa link → `TELEGRAM_FREE_DAILY` lượt free/ngày rồi mời liên kết) + nút "Liên kết Telegram" ở `public/profile.html` (tab Credits). KHÔNG đụng anonymous-create-user (link với tài khoản web đã có; user-only-Telegram để sau).
