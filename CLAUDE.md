...
## 🗞️ Hàng đợi đề tài cạn — `topic-topup` vẫn dùng `JSON.parse` trần trong khi repo đã có bản chắc hơn (2026-08-11, PR này)

Báo cáo COO hằng ngày (routine tự động, đọc DB qua Supabase MCP) phát hiện:
`topic_queue` về **0 dòng chờ** nhiều ngày liền, khiến `cron-khao-luan`/
`cron-master-write` chạy đều `status=ok` nhưng liên tục *"No pending topics"*.
Lần theo `topic-topup` (job nạp đề tài hằng tuần) thì note lần gần nhất (05/08)
ghi: *"LLM không trả được tiêu đề nào"*.

### Căn nguyên
`shapeTopics()` (`lib/content/topic-topup.ts`) gọi thẳng `JSON.parse(raw)` — bản
GIÒN, không bóc được câu dẫn/ghi chú/fence quanh JSON mà model hay chèn khi bị
yêu cầu trả tới 56 tiêu đề (35 nghiên cứu + 21 khảo luận) trong MỘT lượt. Đây
đúng loại lỗi repo đã tự trả giá và tự vá một lần: `lib/llm/json.ts`
(`parseLlmJson`) sinh ra CHÍNH VÌ chuyện này — chú thích đầu file đó ghi thẳng
*"tháng 5 lỗi 34,7%, tháng 6 20,2%, tháng 7 14,9%"* — nhưng `topic-topup.ts`
chưa từng gọi tới nó, vẫn giữ bản `JSON.parse` trần từ trước khi `parseLlmJson`
ra đời.

### Đã làm
- `shapeTopics()` đổi sang gọi `parseLlmJson` (dùng chung, không chép bản hai).
- Nới `maxTokens` 4000 → 6000: 56 tiêu đề tiếng Việt có dấu trong JSON không
  phải lượng output nhỏ — nới cho chắc, dù nguyên nhân thật nằm ở parser chứ
  không phải trần token.

### Verify
`tsc --noEmit` 0 lỗi · `lint` 0 lỗi (73 warning pre-existing) · `prettier
--check` sạch · sanity check `parseLlmJson` trên 10 dạng output (sạch / fence /
câu dẫn / ghi chú / cả hai / ngoặc rác trong lời dẫn / ngoặc trong chuỗi / JSON
cụt / rỗng / không có JSON) — đúng cả 10.
⚠️ **Chưa gọi lại `topic-topup` thật trên prod** để xác nhận hàng đợi được nạp
lại — lượt cron tuần tiếp theo là Thứ Tư 8h VN. Nếu vẫn cạn sau lượt đó thì vấn
đề nằm ở NGUỒN CẦU (GSC/Suggest/seasonal đều rỗng), không phải parser.

### CÒN LẠI
- **`media_posts` vẫn kẹt `queued` — KHÔNG PHẢI bug code, đã xác nhận đọc thẳng
  `lib/media/publish.ts`.** Job chạy đúng mỗi ngày, `error` lưu trên dòng cũ
  nhất nói thẳng lý do: *"Thiếu env FB_PAGE_ID (hoặc MESSENGER_PAGE_ID)"*.
  ⏭️ **Việc tay Henry**: Meta App → Permissions xin thêm `pages_manage_posts`
  (app hiện chỉ có `pages_messaging`, còn Development mode), cấp lại page
  token, đặt `FB_PAGE_ID` + `FB_PAGE_ACCESS_TOKEN` trên Vercel rồi Redeploy.
- Cron `topic-topup` vẫn báo `status=ok` ngay cả khi `inserted=0` do LLM hỏng —
  và Telegram admin chỉ gửi tin khi `inserted>0` nên lượt hỏng hoàn toàn im
  lặng, không ai biết trừ khi soi `note` trong `cron_runs`. Cân nhắc bắn cảnh
  báo riêng khi `note` khác rỗng mà `inserted=0`, tách với ca "không có gì để
  nạp" (dedup hết) vốn dĩ im lặng là đúng — chưa làm trong PR này, phạm vi ban
  đầu chỉ là vá parser.
