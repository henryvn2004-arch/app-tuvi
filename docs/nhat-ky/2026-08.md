...
**KHÔNG đụng** (đây là điểm dễ nhầm — "chat" ở đây CHỈ nói rail, không phải mọi
nơi có chữ "chat"):
...
- **Cũng KHÔNG đụng** hai hàm `handleChat` embedded trong `tubinh/route.ts` và
  `xem-tuoi/route.ts` (action=`chat`, gọi `llmText(...provider:'anthropic')`
  trực tiếp) — về hành vi chúng GIỐNG rail (hỏi-đáp lặp lại), nhưng Henry gõ rõ
  chữ **"rail"**; chưa hỏi lại xem có tính luôn hai chỗ này không. ⚠️ Nếu sau
  này thấy chi phí Opus vẫn cao dù rail đã về Gemini, tra ở ĐÂY trước.
...
### Verify
`npx tsc --noEmit` + `npm run lint` sạch trên 2 file sửa (0 lỗi, cùng 77
