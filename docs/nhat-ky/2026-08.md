...
**Chưa đo được trong container**: egress proxy chặn `query.wikidata.org`,
`qlever.cs.uni-freiburg.de`, `*.wikipedia.org` (403 CONNECT — lỗi nói rõ
*"Host not in allowlist"*, tức **mở được bằng cách thêm host vào egress
allowlist của môi trường**, không phải bất khả). `scripts/measure-celeb-density.mjs`
đã viết sẵn: nhánh T2 chạy OFFLINE (chỉ cần CSV + engine), nhánh T1 cần mạng.
