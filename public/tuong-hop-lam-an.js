/* tuong-hop-lam-an.js — Bộ máy so tuổi / tương hợp HỢP TÁC LÀM ĂN, DETERMINISTIC
   (thuần logic, KHÔNG DOM). Tách từ bản inline của public/xem-lam-an.html
   (calcTuongHop) để app-shell /app/xem-lam-an (public/app-xem-tuoi.html) dùng
   lại được — trước đó app-shell dùng NHẦM window.TuongHop (tuong-hop.js, cung
   Phu Thê/Tử Tức của bản VỢ CHỒNG) cho cả mode hợp tác làm ăn, khiến section
   "Quan Hệ"/"Con Cái" hiện ra thay vì "Đối Tác"/"Giao Tiếp" đúng bối cảnh.

   KHÁC bản tuong-hop.js (vợ chồng) ở ĐÚNG 2 chiều:
     · chiều 4 "Đối Tác" đọc cung Nô Bộc (thay vì Phu Thê)
     · chiều 5 "Giao Tiếp" đọc cung Thân (thay vì Tử Tức)
   5 chiều còn lại (Xét Tuổi/Ngũ Hành/Tư Tưởng/Tính Cách/Tài Chính) + Vận Hạn
   tính giống hệt bản vợ chồng (chỉ khác label "Vận Hạn" thay "Vận Hành").

   ⚠️ KHÔNG đổi bản inline trong xem-lam-an.html — file đó đang chạy tốt, tách
   riêng module này chỉ để app-shell dùng, tránh rủi ro cho tool standalone
   đang bán. Nếu sau này gộp lại làm một nguồn thì phải test cả hai bề mặt.
   Public API: window.TuongHopLamAn = { calcTuongHop, chiRelation, nguHanhRel }. */
(function (root) {
  ...
  function calcTuongHop(lsA, lsB, nameA, nameB) {
    ...
    const pNBA = find(lsA, p => p.cungName === 'Nô Bộc'), pNBB = find(lsB, p => p.cungName === 'Nô Bộc');
    const pThanA = find(lsA, p => p.diaChi === lsA.thanDC), pThanB = find(lsB, p => p.diaChi === lsB.thanDC);
    ...
    const nbc = cungScore(pNBA, pNBB);
    const s4 = { score: nbc.score, w: 0.20, label: 'Đối Tác', detail: (nbc.chiRel && nbc.chiRel.label) || '',
      a: (pNBA ? pNBA.diaChi : '?') + ' · ' + stars(pNBA), b: (pNBB ? pNBB.diaChi : '?') + ' · ' + stars(pNBB),
      palA: pNBA, palB: pNBB, mc: nbc };
    const thc = cungScore(pThanA, pThanB);
    const s5 = { score: thc.score, w: 0.10, label: 'Giao Tiếp', detail: (thc.chiRel && thc.chiRel.label) || '',
      a: (pThanA ? pThanA.diaChi : '?') + ' (Thân ' + (lsA.thanDC || '?') + ')',
      b: (pThanB ? pThanB.diaChi : '?') + ' (Thân ' + (lsB.thanDC || '?') + ')',
      palA: pThanA, palB: pThanB, mc: thc };
    ...
    const s8 = { score: r1((dvA + dvB) / 2), w: 0.05, label: 'Vận Hạn', detail: 'ĐV ' + nameA + ': ' + dvA + '/10 · ĐV ' + nameB + ': ' + dvB + '/10',
