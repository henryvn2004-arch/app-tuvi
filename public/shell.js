...
(function () {
  ...
  var FIXED_TOP = { group: 'Tử Vi Minh Bảo', open: true, items: [
    { id: 'home', label: 'Cuộc trò chuyện mới', href: '/app', icon: 'pen-line' },
    ...
    { id: 'tro-chuyen', label: 'Tất cả cuộc trò chuyện', href: '/app/tro-chuyen', icon: 'message-circle' },
  ] };
  ...
  function wireSidebar(host) {
    ...
    host.addEventListener('click', function (e) {
      ...
      if (k === 'new' && CHAT_HOME) {
        // Đang ở màn chat rồi: "Cuộc trò chuyện mới" dọn tại chỗ, không tải trang.
  ...
  function paintSidebarRecent(list) {
    ...
    if (!arr.length) { el.innerHTML = '<div class="sbn-empty">Các cuộc trò chuyện của bạn sẽ hiện ở đây.</div>'; return; }
    var cur = sessionId;
    el.innerHTML = arr.slice(0, SB_RECENT_MAX).map(function (s) {
      var t = findToolItem(s.tool);
      var name = s.q || s.title || (t && t.label) || 'Cuộc trò chuyện';
  ...
  function joinThay(id) {
    ...
    sys.innerHTML = '<img src="/authors/' + a.id + '.jpg" alt="" width="18" height="18">Thầy ' + esc(a.name) + ' đã vào cuộc trò chuyện';
