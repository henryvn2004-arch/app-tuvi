...
const AUTHOR_SHORT_BIO: Record<string, string> = {
  ...
  'linh-co':    'Linh Cơ tiên sinh thuộc thế hệ học giả được đào tạo trực tiếp từ các bậc thầy lớp trước — phương pháp học truyền tay, không qua lớp học chính thức. Hiện tiên sinh không nhận thêm học trò; những bài viết trong bộ sưu tập này là một trong số ít cách tiếp cận công khai duy nhất với tư tưởng của tiên sinh.',
  ...
  'tam-kinh':   'Tâm Kính tiên sinh là một trong số hiếm học giả hiện nay thực sự thông thạo ba hệ thống — Tử Vi Đẩu Số, Bát Tự Tử Bình và Kỳ Môn Độn Giáp. Cách làm của tiên sinh là đối chiếu cả ba để tìm điểm hội tụ, tạo ra những luận giải toàn diện hơn so với cách tiếp cận từng hệ thống riêng lẻ.',
