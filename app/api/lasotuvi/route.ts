...
import { hasAnySlugAccess, paywallDisabled, hasToppedUp } from '@/lib/billing/credits';
...
async function runPost(request: NextRequest) {
  ...
  if (isPreview && !paywallDisabled()) {
    ...
    // User đã TỪNG nạp tiền thật → bỏ qua hẳn cầu dao 3-lượt-đời (cùng luật
    // với /api/hook-narrative): tầng xem trước là phần thưởng đi kèm, không
    // phải sản phẩm bán riêng, và không được lẫn quota của khách đã trả tiền
    // với quota của khách ẩn danh. SỬA 2026-09-22: `getBalance(...) > 0` SAI
