...
// GET /api/admin/hero-banners?tool=laso&n=15
//
...
//
// Khác illus-images: mỗi tool sinh NHIỀU BIẾN THỂ (n) của CÙNG một prompt để
// duyệt, chưa chốt bức nào — không phải 1 bức cố định/tổ hợp.
//
...
import { HERO_BANNERS, buildHeroBannerPrompt } from '@/lib/media/hero-banner-prompt';
...
export async function GET(req: NextRequest) {
  ...
  const toolId = sp.get('tool') || 'laso';
  const spec = HERO_BANNERS.find((t) => t.id === toolId);
  if (!spec) {
    return json(
      { ok: false, lyDo: `chưa có scene cho tool "${toolId}" trong HERO_BANNERS (lib/media/hero-banner-prompt.ts)` },
      400
    );
  }
  ...
  for (let i = 1; i <= n; i++) {
    ...
    const id = `${toolId}-${String(i).padStart(2, '0')}`;
