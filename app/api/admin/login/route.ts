...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!r.ok) {
      await alertAdminLogin(false, email, ip, data?.error_description || 'sai mật khẩu');
      return err(data?.error_description || 'Sai thông tin đăng nhập', r.status || 401);
    }
    const admin = await getAdminUser(email);
    if (!admin) {
      await alertAdminLogin(false, email, ip, 'không phải tài khoản admin');
      return err('Không có quyền admin.', 403);
    }

    await alertAdminLogin(true, email, ip);
