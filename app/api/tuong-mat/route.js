...
// 4 mùa × undertone
const PC_SEASONS = {
  spring_warm:  { name:'Spring Warm',   vn:'Xuân Ấm',    desc:'Tông da vàng ấm, sáng, tươi — ánh peach/golden',    palette:['Cam đào','Vàng mật','Xanh ngọc ấm','Đỏ san hô','Nâu camel'], avoid:['Xanh lạnh','Tím xám','Đen tuyền'] },
  spring_bright:{ name:'Spring Bright', vn:'Xuân Sáng',  desc:'Tông sáng rõ, contrast cao, màu tươi nổi bật',       palette:['Đỏ tươi','Xanh cobalt','Vàng chanh','Hồng fuchsia','Trắng sáng'], avoid:['Màu xỉn','Pastel nhạt','Nâu đất'] },
  summer_cool:  { name:'Summer Cool',   vn:'Hạ Lạnh',    desc:'Tông hồng lạnh, nhẹ nhàng, da hồng pha xanh',        palette:['Hồng phấn','Lavender','Xanh baby','Xám nhạt','Trắng sữa'], avoid:['Cam nóng','Vàng đất','Nâu ấm'] },
  summer_soft:  { name:'Summer Soft',   vn:'Hạ Dịu',     desc:'Tông lạnh muted, soft contrast — màu pha xám',       palette:['Hồng mauve','Xanh teal muted','Xám xanh','Tím khói','Be lạnh'], avoid:['Màu quá sặc sỡ','Đen thuần','Vàng chói'] },
  autumn_warm:  { name:'Autumn Warm',   vn:'Thu Ấm',     desc:'Tông vàng đất, ấm sâu — da olive/bánh mật',          palette:['Cam đất','Nâu socola','Xanh rêu','Vàng gold','Đỏ gạch'], avoid:['Hồng lạnh','Xanh neon','Bạc lạnh'] },
  autumn_deep:  { name:'Autumn Deep',   vn:'Thu Sâu',    desc:'Tông đất sâu, rich — da nâu ấm, contrast cao',       palette:['Nâu cappuccino','Đỏ burgundy','Xanh forest','Vàng đồng','Đen ấm'], avoid:['Pastel nhạt','Hồng phấn','Xám lạnh'] },
  winter_cool:  { name:'Winter Cool',   vn:'Đông Lạnh',  desc:'Tông lạnh tương phản cao — da trắng xanh hoặc tối',  palette:['Đen','Trắng tinh','Đỏ cherry','Xanh royal','Bạc lạnh'], avoid:['Cam ấm','Vàng đất','Nâu caramel'] },
  winter_deep:  { name:'Winter Deep',   vn:'Đông Sâu',   desc:'Tông lạnh deep, contrast rất cao — da nâu/tối lạnh', palette:['Đen','Trắng tinh','Đỏ tím','Xanh navy','Tím đậm'], avoid:['Vàng nhạt','Cam','Be ấm'] },
};

// Mệnh → personal color season gợi ý
const MENH_SEASON = {
  Kim:  ['winter_cool','summer_cool'],   // Kim = trắng bạc lạnh
  Thủy: ['winter_deep','summer_soft'],   // Thủy = tối sâu lạnh
  Mộc:  ['spring_warm','autumn_warm'],   // Mộc = xanh/vàng ấm
  Hỏa:  ['spring_bright','autumn_deep'], // Hỏa = đỏ/cam nổi bật
  Thổ:  ['autumn_warm','autumn_deep'],   // Thổ = đất ấm
};

// Forked can-chi
const _CAN_PC = ['Giáp','Ất','Bính','Đinh','Mậu','Kỷ','Canh','Tân','Nhâm','Quý'];
const _CHI_PC = ['Tý','Sửu','Dần','Mão','Thìn','Tỵ','Ngọ','Mùi','Thân','Dậu','Tuất','Hợi'];
const _NAP_AM_PC = {
  'Giáp Tý':'Kim','Ất Sửu':'Kim','Bính Dần':'Hỏa','Đinh Mão':'Hỏa','Mậu Thìn':'Mộc','Kỷ Tỵ':'Mộc',
  'Canh Ngọ':'Thổ','Tân Mùi':'Thổ','Nhâm Thân':'Kim','Quý Dậu':'Kim','Giáp Tuất':'Hỏa','Ất Hợi':'Hỏa',
  'Bính Tý':'Thủy','Đinh Sửu':'Thủy','Mậu Dần':'Thổ','Kỷ Mão':'Thổ','Canh Thìn':'Kim','Tân Tỵ':'Kim',
...
function _pcYearToCanChi(y) { return `${_CAN_PC[(y-4+400)%10]} ${_CHI_PC[(y-4+480)%12]}`; }
function _pcGetHanh(y) { return _NAP_AM_PC[_pcYearToCanChi(y)] || 'Thổ'; }

const SP_PERSONAL_COLOR = `Bạn là chuyên gia phân tích Personal Color (màu sắc cá nhân) chuyên về người châu Á.
Nhiệm vụ: phân tích ảnh khuôn mặt để xác định undertone, season type và đưa ra tư vấn màu sắc thực tế.
Trả về JSON THUẦN TÚY — không markdown, không backtick.

8 season type hợp lệ: spring_warm, spring_bright, summer_cool, summer_soft, autumn_warm, autumn_deep, winter_cool, winter_deep

Format:
{
  "undertone": "warm / cool / neutral",
  "undertone_detail": "mô tả chi tiết: tĩnh mạch cổ tay màu gì, da có ánh gì",
  "season": "spring_warm",
  "season_vn": "Xuân Ấm",
  "confidence": "cao / trung bình / thấp",
  "ly_giai": "2-3 câu giải thích tại sao xác định season này — dựa trên tông da, màu mắt, màu tóc quan sát được",
  "ao_so_mi": {
    "top3": ["màu 1 + tên hex", "màu 2 + tên hex", "màu 3 + tên hex"],
    "ly_do": "tại sao 3 màu này làm sáng da và nổi bật nhất",
    "tranh": ["màu nên tránh 1", "màu nên tránh 2"]
  },
  "mau_toc": {
    "top3": ["màu tóc 1", "màu tóc 2", "màu tóc 3"],
    "ly_do": "tại sao các màu tóc này hợp",
    "tranh": ["màu tóc nên tránh 1", "màu tóc nên tránh 2"]
  },
  "phu_kien": {
    "kim_loai": "vàng / bạc / đồng / rose gold — loại nào hợp nhất",
    "kinh": "màu gọng kính hợp nhất",
    "ly_do": "lý do ngắn theo undertone"
  },
...
Quy tắc quan trọng:
- Nếu ảnh thiếu sáng hoặc không rõ tông da: set confidence = "thấp" và ghi rõ vào ly_giai
- hex_palette: 6 màu áo/outfit phù hợp nhất, dạng #RRGGBB
- hex_avoid: 3 màu nên tránh
...
async function handlePersonalColor(body, apiKey) {
  ...
  let menhContext = '';
  let menhHanh = null, menhFull = null, canChi = null, menhSeasons = null;
  if (namSinh && namSinh >= 1900 && namSinh <= 2010) {
    canChi = _pcYearToCanChi(namSinh);
    menhHanh = _pcGetHanh(namSinh);
    menhFull = _NAP_AM_FULL_PC[canChi] || canChi;
    menhSeasons = MENH_SEASON[menhHanh] || ['spring_warm','autumn_warm'];
    const sInfo = menhSeasons.map(s => PC_SEASONS[s]?.vn || s).join(', ');
    menhContext = `

BẢN MỆNH: ${namSinh} (${canChi}) — ${menhFull}, hành ${menhHanh}.
Theo cổ pháp, người mệnh ${menhHanh} thường hợp với season: ${sInfo}.
Nếu kết quả phân tích ảnh cho phép, ưu tiên season này. Nếu tông da thực tế khác hẳn, vẫn trả về season đúng và giải thích thêm về sự tương đồng/khác biệt với mệnh.`;
  ...
  try {
    ...
    if (menhHanh) {
      parsed.menhHanh = menhHanh;
      parsed.menhFull = menhFull;
      parsed.canChi = canChi;
      parsed.menhSeasons = menhSeasons;
    }
    return Response.json(parsed);
  } catch(_) {
