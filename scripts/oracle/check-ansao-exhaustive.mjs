...
// (KNOWN_DIVERGENT_STARS bên dưới — P3, chưa làm) phải làm script này THOÁT
// KHÁC 0. P2 (2026-09) đã sửa 5 bảng tra sai (Đào Hoa/Thiên Quan/Thiên
// Phúc/Thiên Trù/Lưu Hà) — xem docs/nhat-ky/2026-09.md.
//
...
// Điểm lệch ĐÃ GHI trong workplan, còn lại sau khi P2 sửa xong 5 bảng tra
// (Đào Hoa/Thiên Quan/Thiên Phúc/Thiên Trù/Lưu Hà — xem docs/nhat-ky/2026-09.md).
// Lệch ở đây là TRƯỜNG PHÁI (P3, chưa làm), không phải regression. Bất kỳ tên
// nào NGOÀI danh sách này mà lệch vị trí mới là điều bộ dò này cần bắt.
const KNOWN_DIVERGENT_STARS = new Set([
  'Kình Dương', // trường phái: ta cố định, họ đảo theo âm-dương×giới (P3)
  'Đà La',
  'Hóa Khoa', // can Canh: ta gán Thái Âm, họ gán Thiên Đồng — hoán vị với Hóa Kỵ (P3)
...
console.log('-- Lệch ĐÃ BIẾT (trường phái, P3 sẽ sửa, không phải regression) --');
...
if (menhThanCucMismatch.length) {
  failed = true;
  console.error(
    `\n✗ Mệnh/Thân/Cục lệch ở ${menhThanCucMismatch.length} tổ hợp (KHÔNG nằm trong 4 điểm trường phái còn lại):`
  );
...
if (unexpected.length) {
  ...
  console.error(
    `\n✗ LỆCH VỊ TRÍ NGOÀI danh sách 4 điểm trường phái còn lại — ${unexpected.length} tổ hợp, ${byStar.size} tên sao:`
  );
...
console.log(
  '\n✓ check-ansao-exhaustive: mọi lệch đều nằm trong 4 điểm trường phái còn lại. Không có regression bất ngờ.'
);
