...
try {
  ...
  writeFileSync(tsconfigPath, JSON.stringify({
    compilerOptions: {
      ...
      outDir: tmp, rootDir: join(ROOT, 'lib/billing'),
  ...
  const fullPayload = {
    orderCode: 378173725, amount: 50000, desc: 'thanh toan don hang',
    code: '00', counterAccountBankName: 'MB Bank', counterAccountNumber: '123456',
  };
  const oldStyleStr = Object.keys(fullPayload).sort()
    .map((k) => `${k}=${fullPayload[k]}`).join('&');
  assertEqual(
  ...
  const eWalletPayload = { ...fullPayload, counterAccountBankName: null, counterAccountNumber: null };
  const wrongOldSig = hmac(
    Object.keys(eWalletPayload).sort().map((k) => `${k}=${eWalletPayload[k]}`).join('&'),
  ...
  if (correctSig === wrongOldSig) {
    failed = true;
    console.error('✗ payload ví điện tử (có null) PHẢI ký khác bản cũ sai — nếu giống nghĩa là bug đã quay lại');
  } else {
  ...
  assertEqual(
    ...
    verifyPayOSSignature({ data: eWalletPayload, signature: correctSig.slice(0, -1) + (correctSig.at(-1) === '0' ? '1' : '0') }, KEY),
    false,
  );
  assertEqual(
