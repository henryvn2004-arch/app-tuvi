...
const CAN_HOP = {
  'giap-ky':  'Thổ',  'ky-giap':  'Thổ',
  'at-canh':  'Kim',  'canh-at':  'Kim',
  'binh-tan': 'Thủy', 'tan-binh': 'Thủy',
  'dinh-nham':'Mộc',  'nham-dinh':'Mộc',
  'mau-quy':  'Hỏa',  'quy-mau':  'Hỏa',
};
...
export function canRelation(canA, canB) {
  ...
  const hA = CAN[canA].hanh, hB = CAN[canB].hanh;
  const rel = nguHanhRelation(hA, hB);
  if (rel === 'a-khac-b' || rel === 'b-khac-a') return { type: 'khac', desc: 'ngũ hành can tương khắc' };
  if (rel === 'a-sinh-b' || rel === 'b-sinh-a') return { type: 'sinh', desc: 'ngũ hành can tương sinh' };
...
// 6 cặp lục hợp
const LUC_HOP = {
  'ty-suu':   'Thổ', 'suu-ty':   'Thổ',
  'dan-hoi':  'Mộc', 'hoi-dan':  'Mộc',
  'mao-tuat': 'Hỏa', 'tuat-mao': 'Hỏa',
  'thin-dau': 'Kim', 'dau-thin': 'Kim',
  'ti-than':  'Thủy','than-ti':  'Thủy',
  'ngo-mui':  'Thái Dương Thái Âm', 'mui-ngo': 'Thái Dương Thái Âm',
};
...
export function chiRelation(chiA, chiB) {
  ...
  // Tam hợp: cùng group
  const gA = CHI_TO_TAMHOP[chiA], gB = CHI_TO_TAMHOP[chiB];
  if (gA && gA === gB) return { type: 'tam-hop', cucHanh: gA.hanh, desc: `cùng tam hợp ${gA.chis.map(c=>CHI[c].name).join('-')}, hóa ${gA.hanh}` };
  // Lục hợp
  ...
  const hA = CHI[chiA].hanh, hB = CHI[chiB].hanh;
  const rel = nguHanhRelation(hA, hB);
  if (rel === 'a-sinh-b' || rel === 'b-sinh-a') return { type: 'chi-sinh', desc: 'ngũ hành địa chi tương sinh' };
  if (rel === 'a-khac-b' || rel === 'b-khac-a') return { type: 'chi-khac', desc: 'ngũ hành địa chi tương khắc' };
