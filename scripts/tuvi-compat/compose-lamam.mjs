...
export function composeLamAn(analysis, slug) {
  ...
  out.push('');
  out.push(
    `Bài này phân tích tương hợp kinh doanh và hợp tác làm ăn giữa **${tuoiAName}** (mệnh ${naA.napAm} — ${naA.napAmHanh}) và **${tuoiBName}** (mệnh ${naB.napAm} — ${naB.napAmHanh}), đi đủ ba tầng cổ pháp và đưa ra lời khuyên cụ thể về vai trò, rủi ro và phân chia lợi nhuận.`
  );
  out.push('');
  ...
  out.push('');
  out.push(
    `Thiên can ${canA.name} thuộc hành **${canA.hanh}** (${canA.am ? 'âm' : 'dương'}), ${canB.name} thuộc hành **${canB.hanh}** (${canB.am ? 'âm' : 'dương'}). Trong kinh doanh, thiên can phản ánh tư tưởng kinh doanh nền tảng, triết lý làm việc và phương châm xử lý quyết định lớn.`
  );
  out.push('');
  if (canRel.type === 'hop') {
    out.push(
      `Hai can ${canA.name}–${canB.name} ${pick(V.CAN_HOP_DESC, seed, 1)}, hợp hóa thành hành **${canRel.hoaHanh}**. Trong cộng tác kinh doanh, can hợp là yếu tố nền móng quý giá — hai bên có xu hướng nhìn vấn đề từ cùng góc độ, dễ thống nhất chiến lược dài hạn mà không cần tranh luận nhiều.`
    );
    out.push('');
    out.push(
      `Sách "Tử Bình Hạp Hôn" của các bậc thầy đời Tống ghi: "Lưỡng Can Tương Hợp, Đồng Tâm Đồng Đạo" — hai can hợp thì cùng tâm cùng đường. Trong hợp tác làm ăn, đây là yếu tố giảm thiểu xung đột nội bộ — vốn là nguyên nhân hàng đầu khiến các đối tác chia tay sớm.`
    );
  } else if (canRel.type === 'khac') {
    out.push(
      `Hai can ${canA.name}–${canB.name} ${pick(V.CAN_KHAC_DESC, seed, 1)}. ${canA.hanh} và ${canB.hanh} ở thế tương khắc theo ngũ hành.`
    );
    out.push('');
    out.push(
    ...
    out.push(
      `Hai can ${canA.name}–${canB.name} ${pick(V.CAN_SINH_DESC, seed, 1)}. Trong kinh doanh, can tương sinh tạo ra mối quan hệ "Tiền Bối – Hậu Bối" hoặc "Người Hướng Dẫn – Người Thực Thi" — một bên dìu dắt một bên về tầm nhìn, bên kia bù lại bằng sức làm và sự thực thi.`
    );
    out.push('');
    out.push(
    ...
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, cùng can có lợi ở việc đồng cảm nhanh nhưng có rủi ro "Mù Chung Một Hướng" — cả hai cùng nhìn vấn đề từ một góc, dễ bỏ sót các góc nhìn khác. Lời khuyên là cố tình mời thêm cố vấn hoặc nhân sự có tuổi khác hành để cân bằng góc nhìn trong các quyết định lớn.`
    );
  } else {
    out.push(
      `Hai can ${canA.name}–${canB.name} ${pick(V.CAN_BINHHOA_DESC, seed, 1)}. Trong kinh doanh, can bình hòa nghĩa là yếu tố thiên can không tạo ưu thế cũng không tạo cản trở; thành bại phụ thuộc vào hai tầng còn lại và kỹ năng cụ thể.`
    );
  }
  out.push('');
  ...
  out.push('');
  out.push(
    `Địa chi ${chiA.name} mang hành **${chiA.hanh}** (mùa ${chiA.mua}), ${chiB.name} mang hành **${chiB.hanh}** (mùa ${chiB.mua}). Trong kinh doanh, địa chi phản ánh nhịp làm việc, môi trường, đối tượng khách hàng và phương thức vận hành cụ thể.`
  );
  out.push('');
  if (chiRel.type === 'tam-hop') {
    out.push(
      `Hai chi ${chiA.name}–${chiB.name} ${pick(V.TAMHOP_DESC, seed, 2)}, hóa **${chiRel.cucHanh}** cục.`
    );
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, tam hợp địa chi là yếu tố quý hiếm — cổ pháp Tử Bình xem là "Tam Hợp Cục, Tài Lộc Đồng Hành". Biểu hiện thực tế: hai bên dễ ra quyết định chung mà không cần tranh luận dài; khách hàng và đối tác đến với cả hai cùng lúc; thị trường mở ra theo hướng cả hai cùng muốn. Đây là cấu trúc rất phù hợp cho mô hình startup hoặc liên doanh — nơi tốc độ ra quyết định quan trọng hơn sự cẩn trọng từng li.`
    );
  } else if (chiRel.type === 'luc-hop') {
    out.push(
      `Hai chi ${chiA.name}–${chiB.name} ${pick(V.LUCHOP_DESC, seed, 2)}, hóa **${chiRel.hoaHanh}**.`
    );
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, lục hợp là cấu trúc bền bỉ — không bùng nổ nhanh như tam hợp nhưng đi đường dài rất tốt. Phù hợp với các mô hình kinh doanh dài hạn, kinh doanh truyền thống cần sự ổn định, hoặc các nghề đòi hỏi uy tín tích lũy qua nhiều năm.`
    );
    ...
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, tự hình biểu hiện qua việc cả hai cùng có một điểm yếu giống nhau — ví dụ cùng dễ chán việc lâu dài, cùng dễ nóng vội, hoặc cùng hay trì hoãn. Khi điểm yếu chung này bộc phát, không ai bù được cho ai. Lời khuyên: thuê hoặc cộng tác với một bên thứ ba có tính cách bù trừ điểm yếu chung này — ví dụ thuê COO chuyên về vận hành nếu cả hai founder cùng thiên về sáng tạo.`
    );
  } else if (chiRel.type === 'tam-hinh') {
    out.push(`Hai chi ${chiA.name}–${chiB.name} ${pick(V.TAMHINH_DESC, seed, 2)}.`);
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, tam hình thường biểu hiện qua trở ngại đến từ chính sự thân thiết — quá tin nhau dẫn đến không kiểm tra chéo các quyết định, hoặc che chở lẫn nhau khi có vấn đề thay vì xử lý dứt khoát. Lời khuyên: giữ tính chuyên nghiệp trong các vấn đề công việc dù quan hệ cá nhân thân thiết; áp dụng quy trình kiểm toán chéo cho các quyết định lớn về tài chính.`
    );
  } else if (chiRel.type === 'tuong-dong') {
    out.push(`Hai bên cùng chi ${chiA.name}, cùng hành và cùng âm/dương — "Đồng Chi" thuần.`);
    out.push('');
    out.push(
      `Trong kinh doanh, đồng chi mang lại sự đồng cảm tự nhiên nhưng cũng dẫn đến rủi ro "Quá Giống Nhau". Lời khuyên là tận dụng sự đồng điệu để xây dựng văn hóa nội bộ thống nhất, đồng thời cố tình tuyển nhân sự có nền tảng khác biệt để bổ sung góc nhìn.`
    );
  } else if (chiRel.type === 'chi-sinh') {
    out.push(`Hai chi ${chiA.name}–${chiB.name} ${pick(V.CHI_SINH_DESC, seed, 2)}.`);
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, ngũ hành chi tương sinh giúp công việc thường ngày chạy trơn — phân công dễ, giao việc dễ, không có nhiều ma sát trong vận hành. Đây là cấu trúc tốt trung bình, phù hợp với hợp tác có chia vai rõ.`
    );
  } else if (chiRel.type === 'chi-khac') {
    out.push(`Hai chi ${chiA.name}–${chiB.name} ${pick(V.CHI_KHAC_DESC, seed, 2)}.`);
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, chi khắc nhẹ biểu hiện qua khác biệt về nhịp làm việc — một bên thích nhanh, một bên thích chắc; một bên thích họp nhiều, một bên thích làm việc độc lập. Cần thống nhất quy ước về phương thức làm việc từ đầu để tránh va chạm vận hành.`
    );
  } else if (chiRel.type === 'chi-dong') {
    out.push(`Hai chi ${chiA.name}–${chiB.name} ${pick(V.CHI_DONG_DESC, seed, 2)}.`);
    out.push('');
    out.push(
      `Trong hợp tác kinh doanh, cùng hành địa chi tạo cảm thức tương đồng — dễ chọn cùng một loại thị trường, cùng kiểu khách hàng. Tốt cho sự thống nhất chiến lược nhưng cần bổ sung sự đa dạng để mở rộng được.`
    );
  } else {
    out.push(
      `Hai chi ${chiA.name}–${chiB.name} ${pick(V.CHI_BINHHOA_DESC, seed, 2)}. Tầng địa chi trung tính.`
    );
  }
  out.push('');
  ...
  if (naRel.type === 'a-sinh-b' || naRel.type === 'b-sinh-a') {
    ...
    out.push(
      `Hai nạp âm ${naA.napAm} (${naA.napAmHanh}) và ${naB.napAm} (${naB.napAmHanh}) ${pick(V.NAPAM_SINH, seed, 3)}. **${giver}** là nguồn nuôi dưỡng cho **${receiver}** — tốt cho cấu trúc người cấp vốn / người vận hành, hoặc người mở thị trường / người sản xuất.`
    );
    out.push('');
    out.push(
    ...
    out.push('');
    out.push(
      `Trong kinh doanh, nạp âm bình hòa nghĩa là yếu tố vật chất trung tính — thành bại dựa vào kỹ năng cụ thể, quy trình rõ ràng và may mắn thị trường nhiều hơn là dựa vào "duyên mệnh" giữa hai bên.`
    );
  }
  out.push('');
