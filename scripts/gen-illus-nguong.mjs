...
const eng=(new Function('window','globalThis',
  ...
  '\nreturn{convertDuongToAm,anSaoLaSo,formatLaSoV2:window.formatLaSoV2};'))(g,g);
...
function sample(n,seed0){
  ...
  const out={};
  for(let i=0;i<n;i++){
    const year=1960+Math.floor(rnd()*46),month=1+Math.floor(rnd()*12),day=1+Math.floor(rnd()*28);
    const gioIdx=Math.floor(rnd()*12),gender=rnd()<0.5?'nam':'nu';
    try{
      const conv=eng.convertDuongToAm(day,month,year,GIO[gioIdx]); if(!conv?.amLich)continue;
      const al=conv.amLich;
      const ls=eng.anSaoLaSo({ngayAL:al.day,thangAL:al.month,namAL:al.year,canNam:conv.canNam,chiNam:conv.chiNam,gioIdx,gioitinh:gender,namXem:2026});
      const txt=eng.formatLaSoV2(ls); if(!txt||!ls.cungScores)continue;
      const re=/\[([^\]]+)\][^\n|]*\| Luận sao: ([^(]+)\(w:([+-]?[\d.]+)\)/g; let m;
      while((m=re.exec(txt))){
        const cung=m[1].trim(), w=parseFloat(m[3]), s=ls.cungScores[cung]?.tong;
...
let T=0,Tr=0,X=0,MT=0,ALL=0;
for(const [k,v] of Object.entries(test)){
  const {t33,t67}=TH[k];
  let x=0,tr=0,t=0,mt=0;
  v.forEach(o=>{
    const sac = o.rank<=t33?'xau':o.rank>t67?'tot':'trung';
    if(sac==='xau')x++; else if(sac==='tot')t++; else tr++;
    if((sac==='tot'&&o.w<=-0.6)||(sac==='xau'&&o.w>=2))mt++;
  });
  X+=x;Tr+=tr;T+=t;MT+=mt;ALL+=v.length;
  console.log(k.padEnd(12),String(t33.toFixed(2)).padStart(6),String(t67.toFixed(2)).padStart(6),'  |',
    (x/v.length*100).toFixed(1).padStart(5),(tr/v.length*100).toFixed(1).padStart(6),(t/v.length*100).toFixed(1).padStart(6),' |',
    (mt/v.length*100).toFixed(2)+'%');
}
console.log('\nGỘP:  xấu',(X/ALL*100).toFixed(1)+'%  trung',(Tr/ALL*100).toFixed(1)+'%  tốt',(T/ALL*100).toFixed(1)+'%   mâu thuẫn',(MT/ALL*100).toFixed(2)+'%');
