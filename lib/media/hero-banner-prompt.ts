...
/** Mô tả 11 thầy/cô — nguồn thật ở `master-groups.ts`, dùng chung với
 * `tool-avatar-prompt.ts` (icon vuông, thầy/cô cầm deliverable của tool). */
const M = (id: string): string => {
  const g = MASTER_BY_ID[id];
  if (!g) throw new Error(`hero-banner-prompt: thiếu master cho nhóm "${id}"`);
  return g.master;
};
...
The MAIN FOCUS of the scene is a different character: ${M('laso')}. The master is seated cross-legged at a low wooden desk on the right side of the frame. A long scroll lies open flat on the desk, showing a faint circular chart divided into twelve sections with a few soft glowing dots — no readable text, letters or symbols on it. The master points gently at the chart with one hand.
...
The MAIN FOCUS of the scene is a different character: ${M('tu-binh')}, visually distinct in color from the "laso" group's old man. The master sits at a low wooden table on the right side of the frame, carefully arranging four tall bamboo slips upright in a row side by side (representing the Four Pillars), one hand placing a small round yin-yang token beside them.
...
The MAIN FOCUS of the scene is a different character: ${M('than-so-hoc')}. The master sits at a low wooden desk on the right side of the frame, one hand resting on an abacus-like wooden counting frame with a few beads glowing faintly, the other pointing at a sheet of paper showing a few soft glowing numeral marks arranged in a simple grid — no readable digits or text.
...
The MAIN FOCUS of the scene is a different character: ${M('kinh-dich')}. The master sits at a low table on the right side of the frame, in the middle of casting three small bronze coins onto its surface — one coin still spinning — while a faint hexagram of six soft glowing horizontal lines (broken and unbroken) hovers gently above the table, no readable text or symbols.
...
The MAIN FOCUS of the scene is a different character: ${M('xem-tuong')}. The master sits close to the guest on the right side of the frame, gently holding the guest's open palm in both hands, studying its lines with a knowing, gentle smile.
...
The MAIN FOCUS of the scene is a different character: ${M('chiem-tinh-tay')}. The master sits at a small round table on the right side of the frame, holding an antique brass astrolabe up toward a faint circular zodiac wheel of soft glowing stars hovering above the table — no readable text or symbols.
...
The MAIN FOCUS of the scene is a different character: ${M('menh-ly')}. The master sits at a low round wooden table on the right side of the frame, gently gesturing with one open hand toward a long scroll lying flat on the table, showing a faint circular chart divided into twelve sections with a few soft glowing dots — no readable text, letters or symbols on it.
...
The MAIN FOCUS of the scene is a different character: ${M('phong-cach-ai')}. The master stands beside a tall standing mirror on the right side of the frame, gently holding up a few soft fabric swatches and ribbons of different warm colors next to the guest's reflection, comparing them thoughtfully.
...
The MAIN FOCUS of the scene is a different character: ${M('boi-bai')}. The master sits at a small round table on the right side of the frame, fanning out a small hand of ornate but blank-faced cards, one card just being drawn and turned face-up (its face a soft glowing abstract pattern, no readable symbols or text).
