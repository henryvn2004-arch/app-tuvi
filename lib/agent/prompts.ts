...
const FOCUS_TOPICS: Record<string, string[]> = {
  ...
  'bạn bè|nô bộc|nhân viên|đối tác|quý nhân|tiểu nhân|bạn thân|kết bạn|giao du|phản bội|đâm sau lưng|thuộc hạ|cộng sự': ['Nô Bộc'],
  'du lịch|di chuyển|thiên di|nước ngoài|xuất ngoại|định cư|du học|xuất khẩu lao động|đi xa|tha hương|xa quê|làm ăn xa': ['Thiên Di'],
...
function extractKyMonContext(data: any): string {
  ...
  let ctx = '';
  for (const [k, v] of Object.entries(data)) {
    if (k === 'cungs' || v == null || v === '' || Array.isArray(v) || typeof v === 'object') continue;
  ...
  // Lăng kính chủ đề (lib/qimen/chu-de.ts) — cần BanKyMon gốc (`data.cungs`,
  // trang gửi kèm bên cạnh `b.rail` đã format), KHÔNG dựng lại được từ chuỗi
  // `banCung` ở trên. Trả '' nếu câu hỏi không khớp chủ đề nào — không thêm gì.
  if (Array.isArray(data.cungs) && data.cungs.length) {
    const khoi = khoiChuDeKyMon(String(data.cauHoi || ''), data.cungs);
    if (khoi) ctx += khoi;
  }
  return ctx;
}
