...
KHÔNG BAO GIỜ TRÌ HOÃN: tuyệt đối CẤM trả lời bằng lời hứa sẽ làm ("để tôi tra", "để tôi gọi tool", "chờ tôi xem", "tra xong sẽ trả lời", "xin lỗi để tôi tra ngay"). Cần dữ liệu tool → GỌI tool NGAY trong CHÍNH lượt này rồi luận trong cùng câu trả lời. Thông tin ĐÃ có trong DỮ LIỆU LÁ SỐ (cung, sao, cách cục, đại vận đã cung cấp ở trên) → luận THẲNG, KHÔNG gọi tool. Đặc biệt: hỏi về một CÁCH CỤC/cung/sao đã nêu trong lá số (vd cách cục cung Phu Thê) thì luận ngay từ dữ liệu, KHÔNG cần tra vận. Mỗi câu trả lời gửi người dùng PHẢI là luận giải hoàn chỉnh, không phải thông báo ý định.
...
export function execTraVanHan(lasoData: any, input: any): string {
  ...
  if (dv) out += `- KHUNG ĐẠI VẬN ${dv.diaChi} (${dv.tuoiStart}–${dv.tuoiEnd} tuổi)${dv.scoring?.tong != null ? `: điểm ${dv.scoring.tong}/10 ${dv.scoring.flag || ''}` : ''} — chỉ GIỚI HẠN BIÊN ĐỘ (cao = cái tốt rực rỡ/cái xấu đỡ nhẹ; thấp = cái tốt bị kìm/cái xấu nặng thêm), KHÔNG quyết định tốt/xấu của năm.\n`;
