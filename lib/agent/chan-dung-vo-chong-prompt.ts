// lib/agent/chan-dung-vo-chong-prompt.ts
// Tách từ app/api/chan-dung-vo-chong/route.ts (2026-09) — chuyển verbatim,
// KHÔNG viết lại. Lý do tách: scripts/gen-tool-sample.mjs (PDF "Xem mẫu")
// cần gọi ĐÚNG prompt sinh ảnh thật để bản mẫu khớp bản khách trả tiền nhận
// được — chép tay một bản thứ hai là lệch ngay lần đầu đổi prompt mà quên
// sửa cả hai chỗ. route.ts giờ import lại từ đây, không định nghĩa tại chỗ.
import type { SpouseMorphology, PhuTheReadout } from '@/lib/engine/portrait';
import { formatMorphologyForLLM, formatPhuTheForLLM } from '@/lib/engine/portrait';

export const CHAN_DUNG_VO_CHONG_ANALYSIS_SYSTEM_PROMPT =
...
export function buildChanDungVoChongAnalysisPrompt(
  morph: SpouseMorphology,
  phuThe: PhuTheReadout,
  phuTheLuanGiai: string,
  userGender: 'nam' | 'nu'
): string {
  return (
...
export interface ChanDungVoChongAnalysis {
  imagePrompt?: string;
  description?: string;
...
/**
 * Từ kết quả LLM lượt 1 (phân tích) + morph → ghép prompt sinh ẢNH cuối cùng
 * (trang phục/kiểu tóc RANDOM ở đây, KHÔNG để LLM tự chọn — tránh xung đột,
 * xem chú thích gốc ở route.ts). Trả kèm `spouseAge`/`spouseGender` vì route
 * cần hai giá trị này để ghi lịch sử/payload, tính chung một chỗ với tuổi.
 */
export function buildFinalPortraitImagePrompt(
  parsed: ChanDungVoChongAnalysis,
  morph: SpouseMorphology,
  phuTheElement: string,
  userGender: 'nam' | 'nu'
): { finalPrompt: string; spouseAge: number; spouseGender: 'nam' | 'nu' } {
  ...
  return { finalPrompt, spouseAge, spouseGender };
}
